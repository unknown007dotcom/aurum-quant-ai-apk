...
const LiquidityEngine = {
            ...
            if (close > level) {
                ...
                if (isRejected) {
                    return {
                        ...
                        price: level,
                        childClose: close,
                        childDetail: `Closed $${safeToFixed(closeAbove, 2)} above, but upper wick rejection is too large (${((upperWick / totalRange) * 100).toFixed(0)}%)`,
            ...
            if (close < level) {
                ...
                if (isRejected) {
                    return {
                        ...
                        price: level,
                        childClose: close,
                        childDetail: `Closed $${safeToFixed(closeBelow, 2)} below, but lower wick rejection is too large (${((lowerWick / totalRange) * 100).toFixed(0)}%)`,
                        bias: "PENDING — Wait for confirmation",
                        biasDirection: "pending",
                        strength: "Weak"
                    };
    ...
    _findNextTP(allPools, currentPool, eventType, currentClose) {
        ...
        if (candidates.length > 0) {
            return `${candidates[0].name} @ $${candidates[0].safeToFixed(price, 2)}`;
        }
